...
$post_id = intval($_GET["id"]); // show id để đối chiếu với db

// lấy dữ liệu củ lưu vào biến data để chỉnh sửa dễ dàng
$sql = "
SELECT title, content, category_id
FROM posts
WHERE post_id = $post_id"; 
$data = $adminlib->layrow($sql);

$sql = "SELECT * FROM category";
$list_category = $adminlib->get_list($sql);

if (isset($_POST["sua_action"])) {
	$message = $adminlib->sua_post($post_id);
	$error = $message[0];
	$data = $message[1];
}

// show tất cả dữ liệu có trong data
?>
...
<div id="page-wrapper">
	<div id="page-inner">
		...
		<form action="dang_sua.php?id=<?php echo $post_id ?>" method="post" enctype="multipart/form-data">
		...
		<input type="submit" name="sua_action" value="Cập nhật bài viết" class="btn btn-success">
		</form>
